...
function PeerNet(id, ip_address) {
    ...
    this.search = function search(words, cb) {
        words = words.map(function(word) {
            return function(callback) { return word_search(word, callback); };
        });
        async.parallel(words, function(err, results) {
            result = {}
            for (var i in results) {
                for (var j in results[i].results) {
                    if (!(results[i][j].url in result)) {
                        result[url] = {
                            "words": [results[i].word],
                            "frequency": results[i][j].frequency
                        }
                    } else {
                        result[url].words.push(results[i].word);
                        results[url].frequency = results[url].frequency + results[i][j].frequency;
                    }
                }
            }
        });
