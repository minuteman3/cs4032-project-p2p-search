...
function PeerNet(id, ip_address) {
    ...
    var handleJoiningMsg = function handleJoiningMsg(msg) {
        var response = messages.join_relay(msg.node_id, this.id);
    ...
    var handleJoinRelayMsg = function handleJoinRelayMsg(msg) {
        ...
        response = messages.routing_info(msg.node_id, msg.gateway_id, this.ip_address,
                Object.keys(this.routing_table).map(function(nid) {
                    return { node_id: nid, ip_address: this.routing_table[nid] };
                }.bind(this)));
        if (msg.gateway_id === this.id) send(response, msg.node_id);
        else {
            route(msg.gateway_id, function(id) {
                send(response, id, { no_ping: true });
            });
        }
    }.bind(this);
