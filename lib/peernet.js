...
function PeerNet(id, ip_address) {
    ...
    var ackRoute = function ack(suspect_id, id) {
        route(id, function(id) {
            if (id !== this.id) {
                var msg = messages.ack(suspect_id, this.ip_address);
                this.socket.send(msg, 0, msg.length, PORT, this.routing_table[id]);
            }
        }.bind(this));
    }.bind(this);

    var ackDirect = function ackDirect(id, ip) {
        var msg = messages.ack(id, this.ip_address);
    ...
    var handlePingMsg = function handlePingMsg(msg) {
        ackDirect(msg.target_id, msg.ip_address);
    ...
    var handleSearchMsg = function handleSearchMsg(msg) {
        ...
        route(msg.node_id, function(id) {
            if (id !== this.id) send(new Buffer(msg), msg.node_id, { no_ping: true });
        }.bind(this));
    }.bind(this);
