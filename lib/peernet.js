'use strict';

var dgram = require('dgram');
var async = require('async');
var messages = require('./messages.js');
var constants = require('./constants.js');
var hashit = require('./hashit.js');

module.exports = PeerNet;

function PeerNet(id, ip_address, port) {
    this.id = id;
    this.port = port;
    this.ip_address = ip_address;
    this.socket = dgram.createSocket('udp4');
    this.socket.bind(this.port);
    this.routing_table = {};
    this.routing_table[this.id] = {
        ip_address: this.ip_address,
        port: this.port
    };
    this.storage = {};
    this.pending_results = {};
    this.awaiting_acks = {};

    var route = function route(value, cb) {
        var closest = this.id;
        for (var id in this.routing_table) {
            if (Math.abs(id - value) < Math.abs(closest - value)) {
                closest = id;
            }
        }
        cb(closest);
    }.bind(this);

    this.joinNetwork = function joinNetwork(bootstrap_node, bootstrap_port) {
        var msg = messages.join(this.id, this.ip_address, this.port);
        this.socket.send(msg, 0, msg.length, bootstrap_port, bootstrap_node);
    }.bind(this);

    this.leaveNetwork = function leaveNetwork() {
        var msg = messages.leave(this.id);
        async.each(Object.keys(this.routing_table), function(id) {
            this.socket.send(msg, 0, msg.length, this.routing_table[id].port, this.routing_table[id].ip);
        }.bind(this), function(err) {
            if (err) throw err;
        });
    }.bind(this)

    this.search = function search(words, cb) {
        words = words.map(function(word) {
            return function(callback) { return search_word(word, callback); };
        });
        async.parallel(words, function(err, results) {
            var result = {}
            for (var i in results) {
                for (var j in results[i].results) {
                    var url = results[i].results[j].url
                    if (!(url in result)) {
                        result[url] = {
                            "words": [results[i].word],
                            "frequency": results[i].results[j].rank
                        }
                    } else {
                        result[url].words.push(results[i].word);
                        result[url].frequency = result[url].frequency + results[i].results[j].rank;
                    }
                }
            }
            cb(result)
        });
    }.bind(this);

    var search_word = function search_word(word, cb) {
        if (!(word in this.pending_results)) this.pending_results[word] = [];
        var msg = messages.search(word, hashit(word), this.id);
        route(hashit(word), function(id) {
            send(msg, id);
            setTimeout(function() {
                cb(null, {"word": word, "results": this.pending_results[word]});
                this.pending_results[word] = [];
            }.bind(this), 3000);
        }.bind(this));
    }.bind(this);

    this.indexPage = function indexPage(url, words) {
        async.each(words, function(word) {
            index(word, url);
        }.bind(this), function(err) {
            if (err) throw err;
        })
    }

    var index = function index(word, url) {
        var hash = hashit(word);
        route(hash, function(id) {
            if (id !== this.id) {
                var msg = messages.index(hash, this.id, word, url);
                send(msg, id);
            }
        }.bind(this));
    }.bind(this);

    var ackRoute = function ack(suspect_id, id) {
        route(id, function(id) {
            if (id !== this.id) {
                var msg = messages.ack(suspect_id, this.ip_address, this.port);
                this.socket.send(msg, 0, msg.length, this.routing_table[id].port, this.routing_table[id].ip);
            }
        }.bind(this));
    }.bind(this);

    var ackDirect = function ackDirect(id, ip, port) {
        var msg = messages.ack(id, this.ip_address, this.port);
        this.socket.send(msg, 0, msg.length, port, ip);
    }.bind(this);

    var ping = function ping(suspect_id, source_id) {
        route(suspect_id, function(id) {
            if (id === this.id) return;
            var msg = messages.ping(source_id || this.id, suspect_id, this.ip_address, this.port);
            send(msg, id, { no_ping: true });
            this.awaiting_acks[suspect_id] = setTimeout(function() {
                delete this.awaiting_acks[suspect_id];
                delete this.routing_table[suspect_id];
            }, 10000);
        }.bind(this));
    }.bind(this);

    var send = function send(msg, id, options) {
        this.socket.send(msg, 0, msg.length, this.routing_table[id].port, this.routing_table[id].ip);
        if (!(options && options.no_ping)) {
            clearTimeout(this.awaiting_acks[id]);
            this.awaiting_acks[id] = setTimeout(function() {
                ping(id);
            }, 30000);
            this.awaiting_acks[id].reason = JSON.parse(msg.toString()).type
        }
    }.bind(this);

    var handle_incoming = function handle_incoming(buf) {
        var msg = JSON.parse(buf.toString());
        clearTimeout(this.awaiting_acks[msg.sender_id]);
        switch (msg.type) {
            case constants.JOIN_T:
                handleJoiningMsg(msg);
                break;
            case constants.JOINR_T:
                handleJoinRelayMsg(msg);
                break;
            case constants.ROUTING_T:
                handleRoutingMsg(msg);
                break;
            case constants.LEAVING_T:
                handleLeavingMsg(msg);
                break;
            case constants.INDEX_T:
                handleIndexMsg(msg);
                break;
            case constants.SEARCH_T:
                handleSearchMsg(msg);
                break;
            case constants.SEARCHR_T:
                handleSearchResponseMsg(msg);
                break;
            case constants.PING_T:
                handlePingMsg(msg);
                break;
            case constants.ACK_T:
                clearTimeout(this.awaiting_acks[msg.node_id]);
                break;
        }
    }.bind(this);

    var handleJoiningMsg = function handleJoiningMsg(msg) {
        route(msg.node_id, function(id) {
            if (id !== this.id) {
                send(messages.join_relay(msg.node_id, this.id), id);
            }
            this.routing_table[msg.node_id] = {};
            this.routing_table[msg.node_id].ip = msg.ip_address;
            this.routing_table[msg.node_id].port = msg.port
            sendRoutingInfo(this.id, msg.node_id);
        }.bind(this));
    }.bind(this);

    var handleJoinRelayMsg = function handleJoinRelayMsg(msg) {
        route(msg.node_id, function(id) {
            if (id !== this.id) send(msg, id);
        }.bind(this));
        sendRoutingInfo(msg.gateway_id, msg.node_id);
    }.bind(this);

    var sendRoutingInfo = function sendRoutingInfo(gateway_id, node_id) {
        var routing_info = messages.routing_info(node_id
                                               , gateway_id
                                               , this.ip_address,
                Object.keys(this.routing_table).map(function(nid) {
                    return { node_id: nid,
                             ip_address: this.routing_table[nid].ip,
                             port: this.routing_table[nid].port };
                }.bind(this)));
        if (gateway_id === this.id) send(routing_info, node_id);
        else {
            route(gateway_id, function(id) {
                send(routing_info, id, { no_ping: true });
            });
        }
    }.bind(this);

    var handleRoutingMsg = function handleRoutingMsg(msg) {
        async.each(msg.route_table, function(route) {
            this.routing_table[route.node_id] = {
                ip: route.ip_address,
                port: route.port
            };
        }.bind(this), function (err) {
            if (err) throw err;
        });
        if (msg.node_id === this.id) return;
        if (msg.gateway_id === this.id) {
            send(messages.routing_info(msg.node_id, this.id, this.ip_address, Object.keys(this.routing_table).map(function(nid) {
                    return { node_id: nid, ip_address: this.routing_table[nid] };
                }.bind(this))), msg.node_id);
        } else {
            route(msg.gateway_id, function(id) {
                if (id === this.id) {
                    send(new Buffer(msg), msg.node_id);
                } else {
                    send(new Buffer(msg), id);
                }
            }.bind(this));
        }
    }.bind(this);

    var handleLeavingMsg = function handleLeavingMsg(msg) {
        clearTimeout(this.awaiting_acks[msg.node_id]);
        delete this.awaiting_acks[msg.node_id];
        delete this.routing_table[msg.node_id];
    }.bind(this);

    var handleIndexMsg = function handleIndexMsg(msg) {
        ackRoute(msg.target_id, msg.sender_id);
        route(msg.target_id, (function(id) {
            if (id === this.id) {
                if (!this.storage[msg.keyword]) this.storage[msg.keyword] = [];
                for (var link in this.storage[msg.keyword]) {
                    if (this.storage[msg.keyword][link].url === msg.link) {
                        this.storage[msg.keyword][link].rank++;
                        return;
                    }
                }
                this.storage[msg.keyword] = this.storage[msg.keyword].concat({
                    url: msg.link,
                    rank: 1
                });
            } else {
                send(new Buffer(msg), id);
            }
        }).bind(this));
    }.bind(this);

    var handleSearchResponseMsg = function handleSearchResponseMsg(msg) {
        if (msg.word in this.pending_results) {
            this.pending_results[msg.word] = this.pending_results[msg.word].concat(msg.response);
        }
        route(msg.node_id, function(id) {
            if (id !== this.id) {
                send(new Buffer(msg), id);
            }
        }.bind(this));
    }.bind(this);

    var handlePingMsg = function handlePingMsg(msg) {
        ackDirect(msg.target_id, msg.ip_address, msg.port);
        if (msg.target_id !== this.id) {
            ping(msg.target_id, msg.sender_id);
        }
    }.bind(this);

    var handleSearchMsg = function handleSearchMsg(msg) {
        route(msg.sender_id, function(id) {
            send(messages.search_response(msg.word, msg.sender_id, this.id, this.storage[msg.word] || []), id, { no_ping: true });
        }.bind(this));
        route(msg.node_id, function(id) {
            if (id !== this.id) send(new Buffer(JSON.stringify(msg)), id, { no_ping: true });
        }.bind(this));
    }.bind(this);

    this.socket.on('message', handle_incoming);
}
